...
export default class OpenapiRequestCoercer implements IOpenAPIRequestCoercer {
  ...
  constructor(args: OpenAPIRequestCoercerArgs) {
    ...
    const extensionBase = args && args.extensionBase ? args.extensionBase : 'x-openapi-coercion'
...
function buildCoercer(args: any) {
  if (!args.params.length && !args.requestBody) {
    return
  }

  const l = args.isHeaders ? (name: string) => name.toLowerCase() : (name: string) => name

  let properties = args.params.filter(byLocation(args.property))

  if (args.property === 'formData' && args.requestBody) {
    const openapiv3formData = args.requestBody?.content['application/x-www-form-urlencoded']?.schema?.properties
  ...
  const coercers = properties.reduce((acc: any, param: any) => {
...
function buildCoercerForParam(args: any, param: any) {
  const { logger, loggingKey, enableObjectCoercion, coercionStrategy: customStrategy = {} } = args
  const strict = !!param[args.strictExtensionName]

  function getCoercer(type: string) {
    const OBJECT_FORMAT_COERCER = {
      default: (schema: any, input: any) => JSON.parse(input),
      deepObject: (schema: any, input: any) => {
        for (const key of Object.keys(input)) {
          const propertySchema = schema.properties ? schema.properties[key] : schema.additionalProperties
          if (propertySchema) {
    ...
    const COERCION_STRATEGIES = {
      ...
      object: (schema: any, input: any) => {
        ...
        const objectCoercer: any =
          (OBJECT_FORMAT_COERCER[style as keyof typeof OBJECT_FORMAT_COERCER] as any) || OBJECT_FORMAT_COERCER.default
        return objectCoercer(schema, input)
      },

      boolean: (schema: any, input: any) => {
      ...
      integer: (schema: any, input: any) => {
        const result = Math.floor(Number(input))
        return isNaN(result) ? input : result
      },

      number: (schema: any, input: any) => {
        const result = Number(input)
        return isNaN(result) ? input : result
      },

      string: (schema: any, input: any) => String(input),
    }
    ...
    if (customStrategy[type] !== undefined) {
      return (schema: any, input: any) => customStrategy[type](input)
    }
    if (strict && STRICT_COERCION_STRATEGIES[type as keyof typeof STRICT_COERCION_STRATEGIES] !== undefined) {
      return STRICT_COERCION_STRATEGIES[type as keyof typeof STRICT_COERCION_STRATEGIES]
    }
    if (COERCION_STRATEGIES[type as keyof typeof COERCION_STRATEGIES] !== undefined) {
      return COERCION_STRATEGIES[type as keyof typeof COERCION_STRATEGIES]
    }
    const msg = type === undefined ? 'No type has been defined' : `No proper coercion strategy has been found for type '${type}'`

    logger.warn(loggingKey, `${msg}. A default 'identity' strategy has been set.`)
    return (schema: any, input: any) => input
  }
...
function byLocation(location: any) {
  return (param: any) => param.in === location
}

function pathsep(format: any) {
  switch (format) {
    case 'csv':
      return ','
    case 'ssv':
